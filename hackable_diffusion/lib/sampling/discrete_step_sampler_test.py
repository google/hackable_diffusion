...
class UnMaskingStepTest(absltest.TestCase):
  ...
  def test_numerical_no_unmask_gives_all_stay(self):
    ...
    alpha = 0.5
    p_stay, _, p_clean = self._reference_unmasking_routing_weights(
...
class DiscreteDDIMStepTest(absltest.TestCase):
  ...
  def test_numerical_weights_sum_to_one(self):
    ...
    self.assertAlmostEqual((p_stay + p_noise + p_clean) / total, 1.0, places=10)
...
################################################################################
# MARK: IntegratedDiscreteDDIMStep Tests
################################################################################
...
class IntegratedDiscreteDDIMStepTest(absltest.TestCase):
  ...
  def _reference_integrated_posterior(
    ...
    for x0 in range(voc_size):
      q_xt_given_x0 = alpha_t * float(x0 == xt_val) + (1.0 - alpha_t) * pi_xt
...
################################################################################
# MARK: DiscreteFlowMatchingStep Tests
################################################################################
...
class DiscreteFlowMatchingStepTest(absltest.TestCase):
  ...
  def _reference_dfm_routing(self, alpha_s, alpha_t, stoch_coeff=0.0):
    """Compute expected routing weights from the paper formulas."""
    p_clean_raw = (
        (alpha_s - alpha_t) / max(1.0 - alpha_t, 1e-12) * (1.0 + stoch_coeff)
    )
    p_noise_raw = (alpha_s - alpha_t) / max(alpha_t, 1e-12) * stoch_coeff
  ...
  def test_numerical_same_time_all_stay(self):
    ...
    p_stay, _, p_clean = self._reference_dfm_routing(0.5, 0.5, 0.0)
...
################################################################################
# MARK: PriorStep Tests
################################################################################


class PriorStepTest(parameterized.TestCase):
  """Tests for PriorStep sampler."""
  ...
  def setUp(self):
    ...
    self.sampler = discrete_step_sampler.PriorStep(
        corruption_process=self.process,
    )
    self.initial_noise = jnp.zeros((2, 4, 1), dtype=jnp.int32)
  ...
  def test_initialize(self):
    ...
    step = self.sampler.initialize(
        initial_noise=self.initial_noise,
        initial_step_info=step_info,
    )
    chex.assert_trees_all_equal(step.xt, self.initial_noise)
    self.assertEqual(
        step.aux['logits'].shape, (2, 4, self.process.num_categories)
    )
  ...
  def test_update(self):
    ...
    current_step = DiffusionStep(
        xt=self.initial_noise, step_info=step_info, aux={}
    )
    logits = jnp.zeros((2, 4, self.process.process_num_categories))
    prediction = {'logits': logits}
    next_step_info = StepInfo(
        step=1,
        time=jnp.array([0.4, 0.4])[:, None, None],
